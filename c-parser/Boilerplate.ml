...
let rec map_abstract_declarator (env : env) (x : CST.abstract_declarator) =
  (match x with
  | `Abst_poin_decl (v1, v2, v3, v4) -> R.Case ("Abst_poin_decl",
      let v1 = (* "*" *) token env v1 in
      let v2 =
  ...
  | `Abst_paren_decl (v1, v2, v3, v4) -> R.Case ("Abst_paren_decl",
      let v1 = (* "(" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
...
and map_anon_choice_opt_ms_call_modi_decl_decl_opt_gnu_asm_exp_2fa2f9e (env : env) (x : CST.anon_choice_opt_ms_call_modi_decl_decl_opt_gnu_asm_exp_2fa2f9e) =
  (match x with
  | `Opt_ms_call_modi_decl_decl_opt_gnu_asm_exp (v1, v2, v3) -> R.Case ("Opt_ms_call_modi_decl_decl_opt_gnu_asm_exp",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_ms_call_modifier env x
...
and map_anon_choice_param_decl_4ac2852 (env : env) (x : CST.anon_choice_param_decl_4ac2852) =
  (match x with
  | `Param_decl (v1, v2, v3) -> R.Case ("Param_decl",
...
and map_declaration (env : env) ((v1, v2, v3, v4) : CST.declaration) =
  let v1 = map_declaration_specifiers env v1 in
  let v2 =
    map_anon_choice_opt_ms_call_modi_decl_decl_opt_gnu_asm_exp_2fa2f9e env v2
  in
  let v3 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
and map_declaration_declarator (env : env) (x : CST.declaration_declarator) =
  (match x with
  ...
  | `Func_decl_decl (v1, v2, v3, v4) -> R.Case ("Func_decl_decl",
      let v1 = map_declarator env v1 in
      let v2 = map_parameter_list env v2 in
      let v3 =
        (match v3 with
...
and map_expression_not_binary (env : env) (x : CST.expression_not_binary) =
  ...
  | `Exte_exp (v1, v2) -> R.Case ("Exte_exp",
...
and map_field_declarator (env : env) (x : CST.field_declarator) =
  ...
  | `Paren_field_decl (v1, v2, v3, v4) -> R.Case ("Paren_field_decl",
      let v1 = (* "(" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
...
and map_type_qualifier (env : env) (x : CST.type_qualifier) =
  ...
  | `Alignas_qual (v1, v2, v3, v4) -> R.Case ("Alignas_qual",
      let v1 =
        (match v1 with
